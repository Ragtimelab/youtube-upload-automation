...
- [ ] 핵심 컴포넌트 렌더링 확인
  ...
  - [ ] 실시간 데이터 로딩 스피너
- [ ] 콘솔 에러 로그 없음 확인
- [ ] 스크린샷 캡처: `dashboard-page-loaded.png`
...
- [ ] 데이터 로딩 상태 확인
  - [ ] 로딩 스피너 표시 → 데이터 로딩 완료
  - [ ] 스크립트 개수 표시 일치
- [ ] 스크린샷 캡처: `scripts-page-loaded.png`
...
- [ ] 드래그&드롭 영역 스타일링 확인
- [ ] 파일 요구사항 안내 표시
- [ ] 스크린샷 캡처: `upload-page-loaded.png`

#### YouTubeUpload (`/youtube-upload`)
- [ ] 페이지 접근 성공 (HTTP 200)
- [ ] DOM 로딩 완료 대기 (3초 이내)
- [ ] YouTube API 상태 카드 확인
  - [ ] 할당량 정보 표시
  - [ ] 연결 상태 표시
- [ ] 업로드 모드 선택 UI 확인
  - [ ] 단일 업로드 모드
  - [ ] 배치 업로드 모드 토글
- [ ] 스크립트 목록 및 필터 UI 확인
- [ ] 스크린샷 캡처: `youtube-page-loaded.png`
...
- [ ] 시스템 상태 카드들 확인
  ...
  - [ ] WebSocket 연결 상태
- [ ] 실시간 로그 스트림 영역 확인
- [ ] 스크린샷 캡처: `status-page-loaded.png`
...
- [ ] 파이프라인 시각화 컴포넌트 확인
  - [ ] 단계별 프로세스 표시
  - [ ] 상태별 색상 구분
- [ ] 스크린샷 캡처: `pipeline-page-loaded.png`

#### SettingsPage (`/settings`)
- [ ] 페이지 접근 성공 (HTTP 200)
- [ ] DOM 로딩 완료 대기 (3초 이내)
- [ ] 설정 항목들 표시 확인
- [ ] 스크린샷 캡처: `settings-page-loaded.png`

#### HomePage (`/`)
- [ ] 페이지 접근 성공 (HTTP 200)
- [ ] DOM 로딩 완료 대기 (3초 이내)
- [ ] 랜딩 페이지 컨텐츠 확인
- [ ] 스크린샷 캡처: `home-page-loaded.png`
...
### 📝 스크립트 목록 표시 검증
- [ ] 스크립트 데이터 로딩 확인
  - [ ] API 호출 성공: `GET /api/scripts/`
  - [ ] 네트워크 탭에서 응답 시간 500ms 이내
  - [ ] 스크립트 개수 정확 표시 (현재 58개 예상)
- [ ] 테이블 렌더링 완전성 확인
  - [ ] ID, 제목, 상태, 생성일 컬럼 모두 표시
  - [ ] 각 행 데이터 정확성 검증 (최소 5개 행)
  - [ ] 상태 아이콘 적절한 표시 (script_ready, video_ready, error 등)
- [ ] 스크린샷 캡처: `scripts-list-loaded.png`

### 📤 파일 업로드 기능 검증
- [ ] 업로드 버튼 존재 및 접근성 확인
  - [ ] "스크립트 업로드" 버튼 클릭 가능
  - [ ] `fileInputRef.current?.click()` 호출 확인
- [ ] 실제 파일 업로드 프로세스 검증
  - [ ] 테스트 파일 준비: `test-script-playwright.md`
  - [ ] 파일 선택 다이얼로그 동작 확인
  - [ ] 파일 선택 후 업로드 시작 확인
  - [ ] 업로드 진행 중 로딩 스피너 표시
  - [ ] 업로드 완료 후 목록 갱신 확인
- [ ] 업로드 성공 피드백 확인
  ...
  - [ ] 새로운 스크립트가 목록에 추가됨
  - [ ] 파일 입력 필드 초기화됨
- [ ] 스크린샷 캡처: `scripts-upload-success.png`
...
- [ ] 삭제 버튼 접근성 확인
  ...
  - [ ] 버튼 클릭 시 확인 다이얼로그 표시
- [ ] 삭제 프로세스 완전성 검증
  - [ ] 테스트용 스크립트 선택 및 삭제 실행
  - [ ] 확인 다이얼로그에서 "확인" 클릭
  - [ ] API 호출 확인: `DELETE /api/scripts/{id}`
  - [ ] 삭제 완료 후 목록에서 제거 확인
- [ ] 삭제 성공 피드백 확인
  - [ ] 성공 메시지 표시
  - [ ] 목록 자동 갱신
- [ ] 스크린샷 캡처: `scripts-delete-success.png`
...
- [ ] 실제 검색 결과 검증
  ...
  - [ ] 일치하는 스크립트만 표시됨
  - [ ] 검색어 지움 → 전체 목록 복원
- [ ] 스크린샷 캡처: `scripts-search-filtered.png`
...
- [ ] 페이지 이동 동작 검증
  ...
  - [ ] 페이지 번호 변경 확인
  - [ ] "이전 페이지" 클릭 → 원래 데이터 복원
- [ ] 스크린샷 캡처: `scripts-pagination-page2.png`
...
### 📋 스크립트 선택 기능 검증
- [ ] script_ready 상태 필터링 확인
  - [ ] script_ready 상태 스크립트만 표시됨
  - [ ] 다른 상태(video_ready, error 등) 스크립트 제외됨
  - [ ] 선택 가능한 스크립트 개수 표시
- [ ] 스크립트 선택 인터랙션 확인
  - [ ] 스크립트 카드 클릭 시 선택 상태 변경
  - [ ] 선택된 카드 하이라이트 표시
  - [ ] 한 번에 하나만 선택 가능 확인
- [ ] 스크린샷 캡처: `upload-script-selected.png`

### 📁 파일 선택 기능 검증
- [ ] 기본 파일 선택 버튼 확인
  - [ ] "파일 선택" 버튼 클릭 가능
  - [ ] 파일 다이얼로그 열림 확인
  - [ ] 적절한 파일 형식 필터 적용 (video/*)
- [ ] 파일 선택 후 상태 변화 확인
  - [ ] 선택된 파일명 표시
  - [ ] 파일 크기 표시 (MB/GB 단위)
  - [ ] 파일 제거 버튼 활성화
- [ ] 스크린샷 캡처: `upload-file-selected.png`

### 🎯 드래그&드롭 기능 완전 검증
- [ ] 드래그&드롭 영역 초기 상태 확인
  - [ ] 드롭 영역 시각적 구분 표시
  - [ ] 안내 텍스트 표시: "비디오 파일을 드래그하거나 클릭하여 업로드"
- [ ] 드래그 이벤트 핸들링 확인
  - [ ] 파일 드래그 시작 → `dragActive` 상태 true
  - [ ] 드롭 영역 하이라이트 변경 확인
  - [ ] 드래그 벗어남 → `dragActive` 상태 false
- [ ] 실제 파일 드롭 검증
  - [ ] 테스트 비디오 파일 드롭 (또는 시뮬레이션)
  - [ ] `handleDrop` 이벤트 핸들러 실행 확인
  - [ ] 드롭된 파일 정보 표시 확인
- [ ] 스크린샷 캡처: `upload-drag-drop-active.png`

### ⚖️ 파일 크기 검증 로직 확인
- [ ] 8GB 제한 검증 시뮬레이션
  - [ ] 대용량 파일 시뮬레이션 (실제 8GB+ 파일 불가 시)
  - [ ] `validateFileSize` 함수 동작 확인
  - [ ] 에러 메시지 표시: "비디오 파일은 8,192MB를 초과할 수 없습니다"
- [ ] 적절한 파일 크기 검증
  - [ ] 정상 크기 파일 선택 시 검증 통과 확인
  - [ ] 파일 크기 표시 정확성 (MB/GB 단위)
- [ ] 스크린샷 캡처: `upload-size-validation-error.png`

### 🚀 비디오 업로드 진행 검증
- [ ] 업로드 시작 프로세스 확인
  - [ ] 스크립트 선택 + 파일 선택 완료 상태
  - [ ] "비디오 업로드" 버튼 활성화 확인
  - [ ] 버튼 클릭 시 `handleUpload` 함수 실행
- [ ] 업로드 중 UI 상태 확인
  - [ ] 업로드 버튼 비활성화
  - [ ] 로딩 스피너 표시: "업로드 중..."
  - [ ] 진행률 바 표시 (0% → 진행률 증가)
- [ ] 업로드 완료 처리 확인
  ...
  - [ ] 파일 선택 상태 초기화
  - [ ] 다음 업로드를 위한 준비 상태
- [ ] 스크린샷 캡처: `upload-progress-complete.png`

### 🚨 에러 처리 시나리오 검증
- [ ] 잘못된 파일 형식 에러
  - [ ] 비비디오 파일 (예: .txt) 선택 시도
  - [ ] 에러 메시지 표시: "비디오 파일만 업로드할 수 있습니다"
  - [ ] 파일 선택 취소됨
- [ ] 네트워크 에러 시뮬레이션 (가능한 경우)
  - [ ] 업로드 중 네트워크 차단
  - [ ] 에러 메시지 표시 및 재시도 안내
- [ ] 스크린샷 캡처: `upload-error-handling.png`
