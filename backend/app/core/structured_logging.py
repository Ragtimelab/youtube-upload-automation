...
class PerformanceLogger:
    ...
    def log_file_operation(
        ...
        self.logger.info(
            f"File {operation}: {file_path} ({file_size} bytes)", extra=extra
        )
...
def get_upload_logger(script_id: int, upload_type: str) -> ContextualLogger:
    ...
    return get_contextual_logger(
        "upload_processing", script_id=script_id, upload_type=upload_type
    )


def get_api_logger(endpoint: str, method: str = "GET") -> ContextualLogger:
    """API 요청 전용 로거"""
    return get_contextual_logger("api_requests", endpoint=endpoint, method=method)
