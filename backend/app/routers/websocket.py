...
async def handle_client_message(
    connection_id: str, message: dict, user_id: Optional[str], db: Session
):
    """클라이언트 메시지 처리"""
    ...
    if message_type == "subscribe_script":
        ...
        await connection_manager.send_personal_message(
            connection_id, {"type": "connection_stats", "data": stats}
        )
        ...
        await connection_manager.send_personal_message(
            connection_id,
            {"type": "error", "message": f"알 수 없는 메시지 타입: {message_type}"},
        )
...
async def send_script_status(connection_id: str, script_id: int, db: Session):
    ...
    try:
        ...
        if script:
            ...
            await connection_manager.send_personal_message(
                connection_id,
                {"type": "script_status", "script_id": script_id, "data": script_data},
            )
        else:
            await connection_manager.send_personal_message(
                connection_id,
        ...
        await connection_manager.send_personal_message(
            connection_id,
            {"type": "error", "message": f"스크립트 상태 조회 오류: {str(e)}"},
        )
...
async def get_websocket_stats():
    """WebSocket 연결 통계 API"""
    return {"status": "success", "data": connection_manager.get_connection_stats()}
...
async def broadcast_message(message: dict):
    ...
    try:
        ...
        return {"status": "error", "message": f"브로드캐스트 전송 실패: {str(e)}"}


@router.post("/ws/notify/script/{script_id}")
async def notify_script_update(
    script_id: int, update_data: dict, db: Session = Depends(get_db)
):
    """특정 스크립트 업데이트 알림 API"""
    try:
        script_repository = ScriptRepository(db)
        ...
        return {
            ...
            "subscribers": len(
                connection_manager.script_subscribers.get(script_id, [])
            ),
        }

    except Exception as e:
        logger.error(f"스크립트 업데이트 알림 실패: {e}")
        return {"status": "error", "message": f"알림 전송 실패: {str(e)}"}
