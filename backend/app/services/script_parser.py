...
class ScriptParser:
    ...
    def parse_script_file(self, content: str) -> Dict[str, str]:
        ...
        try:
            # 대본 내용 추출
            script_section = self._extract_section(content, "대본", ["메타데이터", "썸네일 제작"])
            if script_section:
                sections['content'] = script_section.strip()
            
            # 메타데이터 섹션 추출 및 파싱
            metadata_section = self._extract_section(content, "메타데이터", ["썸네일 제작"])
            if metadata_section:
    ...
    def _extract_section(self, content: str, section_name: str, next_sections: list) -> Optional[str]:
        """특정 섹션 내용 추출
