...
class ConnectionManager:
    ...
    async def broadcast_message(self, message: dict):
        ...
        logger.info(f"브로드캐스트 완료: {len(connection_ids)}개 연결 - {message.get('type', 'unknown')}")
    ...
    def unsubscribe_from_script(self, connection_id: str, script_id: int):
        ...
        if script_id in self.script_subscribers:
            if connection_id in self.script_subscribers[script_id]:
                self.script_subscribers[script_id].remove(connection_id)
                logger.info(f"스크립트 구독 해제: {connection_id} -> script_id={script_id}")
    ...
    async def broadcast_script_update(self, script_id: int, update_data: dict):
        ...
        if script_id in self.script_subscribers:
            ...
            logger.info(f"스크립트 업데이트 브로드캐스트: script_id={script_id}, 구독자={len(subscribers)}")
    ...
    async def broadcast_upload_progress(self, script_id: int, progress_data: dict):
        ...
        if script_id in self.script_subscribers:
            ...
            logger.debug(f"업로드 진행률 브로드캐스트: script_id={script_id}, 진행률={progress_data.get('progress_percentage', 0)}%")
...
class WebSocketNotificationService:
    ...
    async def notify_video_uploaded(self, script_id: int, script_data: dict):
        """비디오 업로드 완료 알림"""
        await self.manager.broadcast_script_update(script_id, {
            "status": "video_ready",
    ...
    async def notify_youtube_upload_completed(self, script_id: int, script_data: dict, youtube_url: str):
        """YouTube 업로드 완료 알림"""
        await self.manager.broadcast_script_update(script_id, {
            "status": "uploaded",
    ...
    async def notify_upload_error(self, script_id: int, error_message: str, script_data: dict):
        """업로드 오류 알림"""
        await self.manager.broadcast_script_update(script_id, {
            "status": "error",
